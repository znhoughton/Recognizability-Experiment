...
collocations"
shorttitle: "The effects of frequency and predictability on the recognition of *up*"
format:
...
author:
  ...
  - name: 
      literal: Casey Felton
    affiliations:
      - name: University of California, Davis
  - name: 
      literal: Georgia Zellou
    affiliations:
      - name: University of California, Davis
  - name: 
...
[^2]: Indeed, @mollicaHumansStoreMegabytes2019 estimated that, in terms of linguistic information, humans store only somewhere between one million and ten million bits of information, meaning that even their upper estimate is well within the capacity of the brain.
...
On the other hand, predictability has not been directly examined much by the Psycholinguistics literature within the context of holistic multi-word storage [c.f. @odonnellFragmentGrammarsExploring2009]. One such study that did examine the role of predictability in holistic storage was @houghton2023. @houghton2023 examined whether participants were slower to select the first noun in high-predictability compound nouns in locally implausible contexts (i.e., contexts where the first noun in the compound is implausible but where the second noun eliminates the implausibility; see the below sentences) relative to high-predictability compound nouns in locally plausible contexts.
...
$$
\frac{\text{count(Verb+up)}}{\text{count(Verb)} - \text{count(Verb+up)}}
$$ {#eq-logodds}
...
#| label: fig-stimplot
#| fig-cap: "log-predictability by log-frequency (per million) plot of our items. Note that there are no items in the corpus that are both low-frequency and low-predictability, hence the lack of items in the upper left quadrant of the plot."
...
The results of this model are presented in @tbl-gamModelTab and visualized in @fig-gam2dplot1. We found no significant effect of the tensor product smooth.[^5] Although the tensor product smooth for the interaction effect was not significant, it's possible that phrasal verbs and non-phrasal verbs behave differently and that could be obscuring the interaction effect. Thus, we ran an additional model examining whether the interaction effect was different for phrasal verbs versus non-phrasal verbs. The model equation is included below in @eq-gamphrasal:
...
Our results are presented in @tbl-gamModelInterTab and visualized in @fig-gammodelinterplot. The results demonstrated a significant main-effect of predictability (*p* \< 0.05), but no significant effect of frequency (*p* = 0.327), and no significant interaction effect.[^6]
...
#| tbl-cap: "Model results for the generalized Additive Mixed Model cotanining only the interaction between frequency and predictability (@eq-gamminteraction)."
...
summary_table %>%
  mutate(across(where(is.character), ~ gsub(">", "\\\\textgreater{}", .))) %>%
  mutate(across(where(is.character), ~ gsub("<", "\\\\textless{}", .))) %>%
  select(term, everything()) %>%
  rename(` ` = term) %>%
  kable(align = c('l', 'r', 'r', 'r', 'r', 'r', 'r'), digits = 2, row.names = FALSE, booktabs = TRUE, format = "latex", escape = F) %>%
...
#| tbl-cap: "Model results for the Generalized Additive Mixed Model cotaining the interaction between frequency and predictability for phrasal vs nonphrasal verbs (@eq-gamphrasal)."
...
#| tbl-cap: "Model results for the Generalized Additive Mixed Model cotaining Frequency, Predictability, and the interaction between them (@eq-gammFull)."
#table 3
...
#| tbl-cap: "Model results for the Bayesian quadratic regression model containing fixed-effects for frequency, predictability, and their quadratics (@eq-BayesianFullModelSyntax)."
...
colnames(summary_table4) = colnames(summary_table4) %>%
  gsub("%", "\\\\%", .) %>%
  gsub("<", "\\\\textless{}", .) %>%
  gsub(">", "\\\\textgreater{}", .)
...
summary_table4 %>%
  select(term, everything()) %>%
  rename(` ` = term) %>%
  kable(digits = 2, row.names = FALSE, booktabs = TRUE, format = "latex", escape = F) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"), font_size = 11, full_width = FALSE) %>%
  #group_rows(index = setNames(groupings$lengths, groupings$values), indent = TRUE) %>%
  row_spec(0, bold = TRUE) %>%
...
#| label: tbl-brmsFreq
#| tbl-cap: "Results for the Bayesian quadratic regression model containing only frequency and frequency$^2$ (@eq-brmsFreq)."
...
colnames(summary_table5) = colnames(summary_table5) %>%
  gsub("%", "\\\\%", .) %>%
  gsub("<", "\\\\textless{}", .) %>%
  gsub(">", "\\\\textgreater{}", .)
...
#| tbl-cap: "Results for the Bayesian quadratic regression model containing only predictability and predictability$^2$(@eq-brmsPredic)."
...
colnames(summary_table6) = colnames(summary_table6) %>%
  gsub("%", "\\\\%", .) %>%
  gsub("<", "\\\\textless{}", .) %>%
  gsub(">", "\\\\textgreater{}", .)
...
#print(xtable(summary_table6, caption = 'Results for the Bayesian quadratic regression model containing only predictability and $predictability^2$', label = 'tab:brms_predic_quadratic', auto = T), comment = F, caption.placement = 'top', type = 'latex', latex.environments = 'flushleft')
```

```{r, echo = F, out.width = '80%', fig.align = 'center', warning = F, message = F, eval = F}
#| label: fig-FreqOnlyBetaPlot
#| fig-cap: "Plot of the posterior distribution for the beta value of each fixed-effect in the frequency-only quadratic regression model (@eq-brmsFreq). The y-axis contains the different fixed-effects and the x-axis contains the posterior distribution of beta values for the corresponding fixed-effect."
...
#| fig-cap: "Plot of the posterior distribution for the beta value of each fixed-effect in the predictability-only quadratic regression model (@eq-brmsPredic). The y-axis contains the different fixed-effects and the x-axis contains the posterior distribution of beta values for the corresponding fixed-effect."
...
#| fig-cap: "Model predictions for the effects of frequency on reaction times for the frequency-only Bayesian quadratic model (@eq-brmsFreq)."
...
#| fig-cap: "Model predictions for the effect of predictability on reaction times for the predictability-only models (@eq-brmsPredic)."
#| 
...
#| fig-cap: "Plot of the interaction effect between predictability and frequency of the GAM model containing only the interaction between frequency and predictability (@eq-gamminteraction). In the legend, te(predic,freq) refers to the predicted effect of the interaction effect. Thus, the brightness of the coloration denotes the strength of the interaction effect at the point in the graph. Brighter colors denote longer reaction times."

plot(sm(getViz(mod_gam1), 1)) + l_fitRaster() + l_fitContour() + l_points(shape = 19, size = 1.5, alpha = 0.1) + xlab('Predictability (log)') + ylab('Frequency (log)') + ggtitle(NULL) + scale_fill_viridis_c(breaks = c(0.0, 0.1, 0.2), name = 'te(predic,freq)', labels = c('0.0', '0.1', '0.2'))
```

```{r, echo = F, fig.width=6.5, fig.height = 2.5, fig.align = 'center', warning = F, message = F}
#| label: fig-gam2dplot2
#| fig-cap: "Plot of the interaction effect between predictability and frequency of the GAM model containing the interaction between frequency and predictability for phrasal vs nonphrasal verbs (@eq-gamphrasal). Brighter colors denote longer reaction times. The left graph is the predicted effect for phrasal verbs (e.g., pick up), the right graph is the predicted effect for non-phrasal verbs (e.g., walk up)."

nonphrasal_plot = plot(sm(getViz(mod_gam_phrasal_nonphrasal), 1)) + l_fitRaster() + l_fitContour() + l_points(shape = 19, size = 0.7, alpha = 0.1) + xlab('Predictability (log)') + ylab('Frequency (log)') + ggtitle(NULL) + scale_fill_viridis_c(breaks = c(0.0, 0.1, 0.2), name = 'te(predic,freq)', labels = c('0.0', '0.1', '0.2')) +
  theme(
...
phrasal_plot = plot(sm(getViz(mod_gam_phrasal_nonphrasal), 2)) + l_fitRaster() + l_fitContour() + l_points(shape = 19, size = 0.7, alpha = 0.1) + xlab('Predictability (log)') + ylab('Frequency (log)') + ggtitle(NULL) + scale_fill_viridis_c(breaks = c(0.0, 0.1, 0.2), name = 'te(predic,freq)', labels = c('0.0', '0.1', '0.2')) +
  theme(
    axis.title.x = element_text(size = 10),
    axis.text.x = element_text(size = 8),
    axis.title.y = element_text(size = 10),
...
gridPrint(nonphrasal_plot, phrasal_plot, nrow=1)
```

```{r echo = F, message = F, warning = F}
#| label: fig-gammodelinterplot
#| fig-cap: "Plot of the predicted effect of log(predictability) on recongition time for the GAM model specified in @eq-gammFull."
#| out-width: 80%
...
#| fig-cap: "Plot of the posterior distribution for the beta value of each fixed-effect in the full Bayesian quadratic regression model (@eq-BayesianFullModelSyntax). The y-axis contains the different fixed-effects and the x-axis contains the posterior distribution of beta values for the corresponding fixed-effect."
...
The authors would like to thank Dingyi (Penny) Pan for reading and offering comments on a draft of this paper. The authors would also like to thank Vsevolod Kapatsinski and Zara Harmon for providing helpful discussion about the nature of competition in lexical processing.
