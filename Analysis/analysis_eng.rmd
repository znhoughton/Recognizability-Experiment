...
```{r}
data = read.csv('Database_AllParticipants_Recognizability_Experiment2_330703_2023-11-10_18h36.32_faa93908-1176-4bbf-ba5e-5e6fab9181f2.csv', quote = "\"")
...
#data_analysis$OffsetTime[data_analysis$Item==221] = 2.93 #error in the recording of the offset time

#data_analysis = data_analysis %>%
  #mutate(frequency_per_million = Frequency / 2050000000 * 1000000) # need to fix frequency values
...
#data_analysis = data_analysis %>% #should we filter out incorrect responses?
...
```{r}
#data_analysis_offset = data_analysis %>%
  #mutate(response_time = response.rt - OffsetTime) 

#data_analysis_offset_neg_rts = data_analysis_offset %>%
  #filter(response_time < 0)

#data_analysis_offset = data_analysis_offset %>%
  #filter(response_time > 0)
...
#data_analysis_neg_rts = data_analysis %>%
  #filter(response_time < 0)
```
...
```{r}
corpus_size_words = 2111554623
corpus_size_phrase = 1.06491E+12
data_analysis = data_analysis %>%
  mutate(corpus_size = corpus_size_phrase) %>%
  mutate(frequency_per_million = Frequency * 1000000 / corpus_size) %>%
...
```{r}
data_analysis = read_csv('data_analysis_cleaned.csv')
...
cloze_probs = read_csv('cloze_probs.csv')

data_analysis = data_analysis %>%
  mutate(sentences_for_cloze = gsub(' up .*', '', Sentences)) %>%
  left_join(cloze_probs)
#data_analysis$trial = as.factor(data_analysis$trial)
#data_analysis_offset$participant = as.factor(data_analysis_offset$participant)
#data_analysis_offset$Item = as.factor(data_analysis_offset$Item)
#data_analysis_offset$h_heavily_reduced = as.factor(data_analysis_offset$h_heavily_reduced)
...
saveRDS(mod_gam_phrasal_nonphrasal, 'mod_gam_phrasal_nonphrasal.rds')
...
mod_bam_inter = bam(log_rt ~ s(log_freq) + s(log_predic) + s(log_freqxpred) + 
                 Duration + 
                 s(participant, bs = 're') + 
                 s(Item, bs = 're') + 
                 s(log_predic, log_freq, log_freqxpred, participant, bs = 're'),
              method = 'REML',   
              data = gam_data)

saveRDS(mod_bam_inter, 'mod_bam_inter.rds')
...
saveRDS(mod_gam_phrasal, 'mod_gam_phrasal.rds')
...
saveRDS(mod_gam_nonphrasal, 'mod_gam_nonphrasal.rds')
```
...
brms_quadratic_no_phrasal = brm(log_rt ~ log_freq * log_predic * Duration + #fixed-effects
                     ...
                     init = 0,
                     prior = priors,
                     file = 'brms_quadratic_no_phrasal') 
...
bayesian_full_quadratic_english = brm(log_rt ~ log_freq * log_predic * PhrasalVerb * Duration + #fixed-effects
                     ...
                     init = 0,
                     prior = priors,
                     file = 'brms_full_quadratic_english') 
...
bayesian_full_quadratic_english_no_phrasalverb = brm(log_rt ~ log_freq * log_predic * Duration + #fixed-effects
                     ...
                     init = 0,
                     prior = priors,
                     file = 'brms_full_quadratic_english_no_phrasalverb') 
```
...
brms_cloze_quadratic_no_interaction = brm(log_rt ~ log_freq + log_predic + Duration + #fixed-effects
                     ...
                     control = list(adapt_delta = 0.99),
                     prior = priors,
                     file = 'brms_quadratic_cloze_english_no_interaction')  
```
...
```{r include = F, cache = FALSE, eval = F}
plot_3d = plot_gam_3d(
  model = mod_gam1,
  main_var = log_predic,
  second_var = log_freq,
...
#plot_3d_phrasal = plot_gam_3d(
  #model = mod_gam_phrasal,
  #main_var = log_predic,
  #second_var = log_freq,
  #palette = 'bilbao',
  #direction = 1
...
#plot_3d_nonphrasal = plot_gam_3d(
  #model = mod_gam_nonphrasal,
 # main_var = log_predic,
 # second_var = log_freq,
 # palette = 'bilbao',
  #direction = 1
...
#plot_3d_phrasal_nonphrasal_1 = plot_gam_3d(
  #model = mod_gam_phrasal_nonphrasal,
  #main_var = log_predic,
  #second_var = log_freq,
  #palette = 'bilbao',
  ##)
...
```{r mod_gam_3dplot, include = F, cache = FALSE, eval = F}
plot_3d_2 = plot_gam_3d(
  model = mod_gam2,
  main_var = log_predic,
  second_var = log_freq,
  palette = 'bilbao',
...
#### Saving the plots and models for the write-up

```{r eval = F}
saveRDS(object = plot_3d, file = 'plot_3d.rds')
```
